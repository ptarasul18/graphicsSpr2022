...
var VSHADER_SOURCE = 
  ...
  'void main() {\n' +
  'vec4 offsetPos = a_Position + 0.1*vec4(a_Normal, 0.0);\n' +
  '  gl_Position = u_ModelMatrix * offsetPos;\n' +
  '  gl_PointSize = 10.0;\n' +
...
//   'uniform mat4 u_ViewMatrix;\n' +
  ...
  'uniform float C;\n' +
  //TODO can I modify a_Position in place? 
  'void main() {\n' +
  'vec4 modPos = vec4((a_Position.x*(1.0+aConst*sin(bConst*a_Position.z + C))), (a_Position.y*(1.0+aConst*sin(bConst*a_Position.z + C))), a_Position.z, 1.0);\n' +
  '  gl_Position = u_ModelMatrix * modPos;\n' +
  '  gl_PointSize = 10.0;\n' +
  '  v_Color = a_Color;\n' +
  '}\n';
...
//   viewMatrix.setLookAt(5.0, 5.0, 3.0, -1.0, -2.0, -0.5, 0, 0, 1);
...
//-----------------  
  ...
  var tick = function() {
	...
	//update C?
	var C = gl.getUniformLocation(gl.program, 'C');
	if (!C) { 
		console.log('Failed to get the storage location of c');
		return;
	}
	gl.uniform1f(C, g_C);
...
// shapes.
  ...
  //shivering constants
  var aConst = gl.getUniformLocation(gl.program, 'aConst');
  if (!aConst) { 
    console.log('Failed to get the storage location of a');
    return;
  }
//   gl.uniform1f(aConst, 0.3); 
  gl.uniform1f(aConst, 0.3); 

  var bConst = gl.getUniformLocation(gl.program, 'bConst');
  if (!bConst) { 
    console.log('Failed to get the storage location of b');
    return;
  }
  gl.uniform1f(bConst, Math.PI);
  gl.uniform1f(bConst, 0.0);  
...
//TEMPORARY: 1/10th size camera pose to see what's in CVV locations
  modelMatrix.lookAt( ??, ??, ??,	// center of projection
                      ...
                      ??, ??, ??);	// View UP vector.
*/
// modelMatrix.lookAt(0.20, 0.25, 0.25, 0, 0, 0, 0, 1, 0);
modelMatrix.lookAt(3.0, 3.0, 1.0, -1.0, -1.0, -0.5, 0, 0, 1);
// modelMatrix.scale(0.1, 0.1, 0.1);
